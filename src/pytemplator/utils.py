...
def cd(new_dir):  # pylint: disable=invalid-name
    ...
    Implementation taken from https://stackoverflow.com/a/24176022
    """
...
def generate_context_from_json(initializer, context):
    """Generate the context from a json file.

    Same behavior as cookiecutter.
    """
    with open(initializer) as file:
        questions = json.load(file, object_pairs_hook=OrderedDict)
    for key, value in questions.items():
        question = key.replace('-', ' ').replace('_', ' ')
        default_value = Template(value).render(context)
        context[key] = (
            input(f'{question} [{default_value}]') or default_value
        )
    return context
...
def render_templates(templates, root_directories, context):
